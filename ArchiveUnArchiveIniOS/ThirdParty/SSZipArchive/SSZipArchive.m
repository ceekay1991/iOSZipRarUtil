...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error delegate:(id<SSZipArchiveDelegate>)delegate {
	...
	do {
		@autoreleasepool {
            ...
            strPath = [NSString stringWithCString:filename encoding:NSUTF8StringEncoding];
            if (!strPath &&delegate && [delegate respondsToSelector:@selector(zipArchiveReEncodeFilename:)])
            {
                strPath = [delegate zipArchiveReEncodeFilename:filename];
                if (!strPath.pathExtension) {
                    
                    unzCloseCurrentFile(zip);
                    ret = unzGoToNextFile(zip);
                    continue;
                }
                //strPath =  [NSString stringWithFormat:@"%ld.%@",(long)currentFileNumber,strPath.pathExtension];
            }
	...
	} while(ret == UNZ_OK && ret != UNZ_END_OF_LIST_OF_FILE && !([delegate respondsToSelector:@selector(isStopUnZipFile)]&&[delegate isStopUnZipFile]));
...
//    }
	...
	// Message delegate
	if (success && [delegate respondsToSelector:@selector(zipArchiveDidUnzipArchiveAtPath:zipInfo:unzippedPath:)] && ![delegate isStopUnZipFile]) {
